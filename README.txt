...
★★★【下次打开优先处理】机械结构改造 ★★★
------------------------------------------------------------
  把传感器往后挪，缩短"传感器 → 后部驱动轮轴"的距离
    当前：140mm（轮距100mm）
    目标：压到70mm以内，建议留余量做到 30~50mm

  原因：赛道发卡弯最大R=70~75mm。几何推导：小车原地掉头时，传感器
        能画出的最小转弯半径 = 传感器到驱动轮轴的距离，跟差速算法、
        轮距都无关。现在140mm > 70mm，纯闭环循线在这个结构下物理上
        过不去R70弯（不是代码/阈值能解决的问题）。
        已排除"软件盲转"方案——赛道弯道大小不一、不全是180°调头，
        开环预设动作不通用，改结构更稳妥。

  改完结构之后要做：
    1. 传感器离地高度若变化，CH2~CH6阈值可能要重新校准
...
【待完成】
------------------------------------------------------------
  - 机械结构改造：传感器到驱动轮轴距离缩短到70mm以内（见文档最上方，R70发卡弯必须项）
